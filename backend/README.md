...
## Практические задания LMS

REST API `/api/v1/lms/submissions/` принимает ответ сотрудника текстом и/или multipart-
файлом. Размер, MIME-типы и расширения ограничиваются deployment-настройками `LMS_*`.
Storage URL не публикуется: вложение скачивается
через защищённый action `/{id}/attachment/` только автором ответа, назначенным куратором,
методологом или HR.

Очередь куратора доступна через `/review-queue/`, результат сохраняется через
`/{id}/review/`. Повторная отправка ответа удаляет прежнюю оценку и возвращает работу в
очередь; повторная оценка одной версии отклоняется контролируемой ошибкой.
